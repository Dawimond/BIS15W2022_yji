...
penguins %>%
  ...
  summarize(mean_bill_length=mean(bill_length_mm, na.rm=T), mean_bill_depth=mean(bill_depth_mm, na.rm=T), mean_flipper_length=mean(flipper_length_mm, na.rm=T), mean_mass=mean(body_mass_g, na.rm=T), n=n())
```
...
penguins%>%
  group_by(species)%>%
  summarise(mean_bill_length_all = mean(bill_length_mm, na.rm = T))
```

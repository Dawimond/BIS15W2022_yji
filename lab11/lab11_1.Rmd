...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "mistyrose3", bins=80)+
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "mistyrose3")+
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(alpha = 0.4, color = "black", fill = "mistyrose3")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))+
  labs(title = "Distribution of HRA")
```
...
homerange <- homerange %>% 
             mutate(range_category = case_when(log10.hra <= 3.65 ~ "very small",
                                               log10.hra > 3.65 & log10.hra <= 4.60 ~ "small",
                                              log10.hra > 4.60 & log10.hra <= 6.00 ~ "medium",
                                              log10.hra > 6.00 ~ "large"))
```

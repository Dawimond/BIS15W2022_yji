...
```{r}
elephants %>% 
  ggplot(aes(x=age, y=height, na.rm = T))+
  geom_point(na.rm = T) + geom_smooth(method="lm")
```
...
homerange %>% 
  ...
  labs(title = "# Elephants by Thermal Regulation",
       x = "Thermal Regulation",
       y = NULL) +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.mass, y=mean.hra.m2, na.rm = T, size=log10.mass))+
  geom_jitter(size=1, color="navyblue", alpha=0.5)+
  scale_y_log10()+
